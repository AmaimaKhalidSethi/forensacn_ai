...
# ---------------------------------------------------------------------------
# AI routes — explanation + chat
# ---------------------------------------------------------------------------

@app.route("/api/explain/<scan_id>")
def api_explain(scan_id):
    """Return AI-generated plain-English explanation of a scan as JSON."""
    from flask import jsonify
    try:
        _validate_scan_id(scan_id)
    except ValueError:
    ...
    results = analyzer.get_scan_results(scan_id)
    if not results:
        return jsonify({"error": "Scan not found."}), 404

    from ai_utils import explain_scan
    data = explain_scan(results)
    return jsonify(data)


@app.route("/api/chat/<scan_id>", methods=["POST"])
@limiter.limit("60 per hour")
def api_chat(scan_id):
    """Single-turn forensic chatbot endpoint.
    ...
    # Sanitise history: keep only valid role/content pairs, cap at last 10 turns
    clean_history = [
    ...
    from ai_utils import chat as ai_chat
    reply = ai_chat(user_message, clean_history, results)

    if reply.startswith("ERROR:"):
        return jsonify({"error": reply[6:].strip()}), 503
